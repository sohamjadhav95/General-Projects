...
class VoiceEmotionDetector:
    ...
    def analyze_file(self, input_source, output_dir=None, apply_smoothing=True, window_size=3):
        ...
        all_results = []
        for i in range(5):
            logger.info(f"Running iteration {i+1}/5...")
            # Analyze emotions
        ...
        # Calculate average dominant emotion
        dominant_emotions = [r['dominant_emotion'] for r in all_results]
        emotion_frequencies = {}
        for emotion in dominant_emotions:
            emotion_frequencies[emotion] = emotion_frequencies.get(emotion, 0) + 1
        averaged_dominant_emotion = max(emotion_frequencies.items(), key=lambda x: x[1])[0]
        
        # Use the last iteration's results for visualization
        ...
        if output_dir:
            ...
            self.visualize_emotions(
                ...
                confidence_matrix, 
                last_results['timestamps'], 
                output_path=viz_path,
                smoothed_emotions=last_results['smoothed_emotions'],
                title=f"Emotion Analysis: {base_filename}"
            )
            
            # Save analysis results as CSV
            results_df = pd.DataFrame({
                'timestamp': last_results['timestamps'],
                'emotion': last_results['emotions']
            })
            if last_results['smoothed_emotions']:
                results_df['smoothed_emotion'] = last_results['smoothed_emotions']
            ...
            conf_df['timestamp'] = last_results['timestamps']
        ...
        # Return results
        return {
            'averaged_dominant_emotion': averaged_dominant_emotion,
...
def main():
    ...
    if results:
        ...
        print(f"Duration: {results['all_iterations'][0]['duration']:.2f} seconds")
        print(f"Averaged dominant emotion: {results['averaged_dominant_emotion']}")
        print("\nAnalysis complete!")
