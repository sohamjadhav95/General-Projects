...
def find_and_fill_visible_fields(img, rect, fields_to_fill):
    ...
    for i, text in enumerate(ocr_data["text"]):
        if text.strip():  # Ignore empty text
            detected_word = text.strip().lower()
            
            # Match detected fields with form_data keys (case-insensitive, split into parts)
            for field, value in list(fields_to_fill.items()):
                field_parts = field.lower().split()  # Split field into parts, e.g., ["first", "name"]
                
                # Check if all parts of the field are present near each other
                matches = []
                for part in field_parts:
                    for j, ocr_word in enumerate(ocr_data["text"]):
                        if part in ocr_word.lower():
                            matches.append((ocr_data["left"][j], ocr_data["top"][j], 
                                            ocr_data["width"][j], ocr_data["height"][j]))
                            break
                
                # If all parts are detected, proceed to click and fill
                if len(matches) == len(field_parts):
                    # Calculate the average bounding box to click
