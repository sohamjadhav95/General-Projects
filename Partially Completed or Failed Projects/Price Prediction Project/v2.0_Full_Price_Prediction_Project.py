...
print("\nModel Performance:")
print(f"Accuracy Score: {accuracy * 100:.2f}%")  # Print accuracy in percentage
...
# Start with the last closing price in the training set
last_train_price = X_train["Close"].iloc[-1]
predicted_prices = [last_train_price]

# Iterate through the test set and calculate predicted prices
for i in range(len(X_test)):
    if y_pred[i] == 1:  # Predicted price will go up
        predicted_prices.append(predicted_prices[-1] * (1 + X_test["Returns"].iloc[i]))
    else:  # Predicted price will go down
        predicted_prices.append(predicted_prices[-1] * (1 - X_test["Returns"].iloc[i]))

# Drop the first value (last_train_price) to align with X_test
predicted_prices = predicted_prices[1:]

# Add predicted prices to the test data
X_test["Predicted_Close"] = predicted_prices
...
# Visualization 3: Actual vs Predicted Closing Prices
plt.figure(figsize=(15, 6))
plt.plot(X_test.index, X_test["Close"], label="Actual Closing Price", color="blue")
plt.plot(X_test.index, X_test["Predicted_Close"], label="Predicted Closing Price", color="red", linestyle="--")
